...
+function ($) {
    ...
    var apiCall = function (url, params, callback) {
        ...
        // Data.apiKey MUST SET ON EVERY PAGE WITH VALID API KEY
		if (!Data.apiKey)
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
				...
				xhr.setRequestHeader('Access-Control-Allow-Headers', 'Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept');
	...
	// BUILDING PROPER URL OF API CALL LIKE /api/data/v1/.../...
    Data.api = {
        // VERSION 1
        v1: {
            // STOCK METHODS
			stock: function (stock) {
                // GET STOCK - /api/data/v1/:otasSecurityId
				var toReturn = function (callback) {
                    return apiCall(otasBase.baseUrl + "/v1/stock/" + stock + "/", null, callback);
                }
				// GET DAILY FLAGS - /api/data/v1/:otasSecurityId/dailyFlags
                toReturn.dailyFlags = function (params, callback) {
                    return apiCall(otasBase.baseUrl + "/v1/stock/" + stock + "/dailyFlags", params, callback);
                }
                // GET DAILY SERIES
                toReturn.dailySeries = function (params, callback) {
                    return apiCall(otasBase.baseUrl + "/v1/stock/" + stock + "/series", params, callback);
                }
    ...
    Flag.prototype._createText = function (text, textCls, sizeCls) {
        console.log(':::_createText:::', text + " " + textCls + " " + sizeCls);
        var div = $(document.createElement("div")).addClass('dailyflag');
    ...
    IntradayFlag.prototype.createFlag = function () {
        ...
        if (this.options.tooltip) {
            ...
            jQuery(flagElem).tooltip(tooltipopts);
        }
    ...
    var DailyFlag = function (element, options) {
        console.log(":::DailyFlag options:::", options);
        this.$element = $(element);
        this.flag = options.flag;
    ...
    DailyFlag.prototype.createFlag = function() {
        console.log(':::createFlag called:::');
		var flagElem;
        var flag = this.flag;
		console.log(':::flag:::',flag);
		var tooltipopts = DailyFlag.TOOLTIP_OPTIONS;
		console.log(':::tooltipopts:::', tooltipopts);
        if (!flag)
            flagElem = null;
        else {            
			console.log(':::flag type:::',flag.flagType.toLowerCase());
			switch (flag.flagType.toLowerCase()) {
                ...
                case "earningsestimate1m":          // earnings estimate 1 month flag
                    console.log(':::earningsestimate1m case entered:::');
                    console.log(':::flag.percentile:::', flag.percentile);
                    if (flag.percentile < DailyFlag.conditions.earnings.percentile) {
                        flagElem = this._createLrgDownNeg();
                        this.direction = Enums.buySell.SELL;
                        this.pvalue = flag.percentile * percmult;
                        console.log(':::earningsestimate1m < less entered:::');
                        ...
                        this.direction = Enums.buySell.BUY;
                        this.pvalue = 1.0 - (flag.percentile * percmult);
                        console.log(':::earningsestimate1m > greater entered:::');
                    }
                    else
                        flagElem = null;
                    tooltipopts = $.extend({}, tooltipopts, { title: this._getPercentileTooltipContent() });
			    ...
			    case "valuation":               // valuation flag
			        console.log(':::into valuation case:::');
			        console.log(':::flag.zscore:::', flag.zscore);
                    if (flag.zscore < -(DailyFlag.conditions.valuation.higherZscore || DailyFlag.higherZscore)) {
                        ...
                        console.log(':::flag.zscore < -(DailyFlag.conditions.valuation.higherZscore || DailyFlag.higherZscore):::');
                    }
                    else if (flag.zscore < -(DailyFlag.conditions.valuation.lowerZscore || DailyFlag.lowerZscore)) {
                        flagElem = this._createSmlDownPos();
                        this.direction = Enums.buySell.BUY;
                        this.pvalue = Math.stats.distributions.normal.cdf(flag.zscore);
                        console.log(':::flag.zscore < -(DailyFlag.conditions.valuation.lowerZscore || DailyFlag.lowerZscore):::');
                    }
                    ...
                    break;
                case "volume":
                    console.log(':::volume case:::');
                    console.log(':::flag.zscore:::', (DailyFlag.conditions.volume.higherZscore || DailyFlag.higherZscore));
                    if (flag.zscore < (DailyFlag.conditions.volume.higherZscore || DailyFlag.higherZscore)) {
                        ...
                        this.direction = null;
                        this.pvalue = 1.0 - Math.stats.distributions.normal.cdf(flag.zscore);
						console.log(':::volume lrg 1:::');
                    }
                    else if (flag.zscore > (DailyFlag.conditions.volume.lowerZscore || DailyFlag.lowerZscore)) {
                        flagElem = this._createText("High", "volume-signal", "sml");
                        this.direction = null;
                        this.pvalue = 1.0 - Math.stats.distributions.normal.cdf(flag.zscore);
						console.log(':::volume sml 1:::');
                    }
        ...
        console.log(':::flagElem in createFlag:::', flagElem);
        return flagElem;
    }
    ...
    DailyFlag.prototype.init = function () {
        var flag = this.flag;
        console.log(':::flag in DailyFlag:::', flag);
        var flagElem = this.createFlag();
        if (!flagElem)
            flagElem = this._createEmpty();
    ...
    var DailyFlagRow = function (element, options) {
        console.log(":::DailyFlagRow options:::", options);
        this.$element = $(element);
    ...
    DailyFlagRow.prototype.init = function () {
        ...
        console.log(':::DailyFlagRow:::');
        //console.log(':::this.flags:::', this.flag[this.options.flagOrder[2]]);
        // iterate over flags
        var dfs = new Array();
        for (var i = 0; i < this.options.flagOrder.length; i++) {
            //console.log(this.flags[this.options.flagOrder[i]]);
            //console.log(':::length:::', this.options.flagOrder.length[2]);
            var df = new DailyFlag(null, { flag: this.options.flagOrder[i], header: true, tooltip: this.options.tooltip });
            var flagElem = df.createFlag();
            console.log(':::flagElem in DailyFlagRow:::', flagElem);
            if (flagElem)
                this.$element.append(flagElem);
            dfs.push(df);
        }
    ...
    StockDescriptor.prototype.init = function () {
		console.log(':::StockDescriptor called:::');
